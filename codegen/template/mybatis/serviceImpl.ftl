...
public class ${mainTable.variables.class}ServiceImpl implements ${mainTable.variables.class}Service {
	...
	public void doDelete${mainTable.variables.class}(Long ${mainTable.variables.class?uncap_first}Id) {
		${mainTable.variables.class?uncap_first}Dao.delById(${mainTable.variables.class?uncap_first}Id);
	}

	@Override
	public ${mainTable.variables.class} find${mainTable.variables.class}ById(Long ${mainTable.variables.class?uncap_first}Id) {
		return (${mainTable.variables.class})${mainTable.variables.class?uncap_first}Dao.getById(${mainTable.variables.class?uncap_first}Id);
	}
	...
	public ${mainTable.variables.class} find${mainTable.variables.class}By${mainTable.parentClassName}Id(Long ${mainTable.parentClassName?uncap_first}Id) {
		List<${mainTable.variables.class}> ${mainTable.variables.class?uncap_first}List = ${mainTable.variables.class?uncap_first}Dao.getBySqlKey("getBy${mainTable.parentClassName}Id", ${mainTable.parentClassName?uncap_first}Id);
		if (${mainTable.variables.class?uncap_first}List == null || ${mainTable.variables.class?uncap_first}List.isEmpty()) {
			return null;
		}
		return ${mainTable.variables.class?uncap_first}List.get(0);
	}
	</#if>
}
