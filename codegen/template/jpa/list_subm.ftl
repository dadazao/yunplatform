			...
			<c:forEach items="${'${'}pageResult.content${'}'}" var="${mainTable.variables.class?uncap_first}" varStatus="status"><#noparse>
                ...
                <c:if test="${status.count%2!=0}">
                    ...
                    <td class="tdClass" style="width: 10%; border-bottom:1px #ededed solid; border-right:1px #ededed solid;">
						 &nbsp;
						 <a onclick="ns.${mainTable.parentClassName?uncap_first}.loadSubEdit${mainTable.variables.class}('<#noparse>${</#noparse>${mainTable.variables.class?uncap_first}.id<#noparse>}</#noparse>')" style="cursor: hand"><span>维护</span></a>
						 &nbsp;
					</td>
                </tr>
            </c:forEach>
