...
if(json.message&&alertMsg)alertMsg.error(json.message,json.options,json.title);}else if(json.statusCode==DWZ.statusCode.timeout){
...
return this.optional(element)||/^((\d{3,4}[\s\-])|(\d{3,4}))?\d{3,8}(([\s\-]\d{1,4})|(\d{1,4}))?$/.test(value);},"Please enter a valid telephone number.");
$.validator.addMethod("mobile",function(value,element){
return this.optional(element)||/^(13|15|18)\d{9}$/.test(value);},"Please enter a valid mobile number.");
$.validator.addMethod("postcode",function(value,element){
return this.optional(element)||/^[1-9]\d{5}$/.test(value);},"Please enter a valid postcode number.");
$.validator.addMethod("identitycard",function(value,element){
return this.optional(element)||/^(([1-9]\d{5}(19|20)\d{2}(([0][1-9])|([1][012]))(([0][1-9])|([12][0-9])|([3][01]))\d{3}([0-9]|[x]|[X]))|([1-9]\d{5}\d{2}(([0][1-9])|([1][012]))(([0][1-9])|([12][0-9])|([3][01]))\d{3}))$/.test(value);},"Please enter a valid identitycard number.");
$.validator.addMethod("bankcard",function(value,element){
return this.optional(element)||/^\d{16,}(\s)*$/.test(value);},"Please enter a valid bankcard number.");
$.validator.addMethod("pinyin",function(value,element){
return this.optional(element)||/^[A-Za-z0-9_][A-Za-z0-9_]+$/.test(value);},"Please enter a valid pinyin.");
...
continue;}else{
valid=false;}}
return this.optional(element)||valid;},"Please make sure that all the addresses are valid");
...
$.validator.autoCreateRanges=false;}})(jQuery);(function($){
...
top:($(window).height()/2-$(this._boxId).height())+$(window).scrollTop()+"px",
...
$(this._boxId).animate({
top:($(window).height()/2-$(this._boxId).height())+$(window).scrollTop()+"px",
left:$(window).width()/2+$(window).scrollLeft()+"px"},1,function(){
$(this).remove();});
$(this._bgId).hide();},
...
ypArtDialog(msg,options);}};(function($){
...
return iframeCallback(form,selfIframeDialogAjaxDone);}else{
if(!$form.valid()){
return false;}
$.ajax({
...
$(".jumpto",$this).children().first().val((currentPage-1)==0?1:(currentPage-1));}}else{
...
if(json.statusCode==DWZ.statusCode.ok){
if(json.navTabId){
refreshList(json);}else if(json.rel){
navTabPageBreak({},json.rel);
DWZ.ajaxDone(json);}
if("closeCurrent"==json.callbackType){
...
break;
case 'date':
html='<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" format="'+field.patternDate+'" size="'+field.size+'"/>'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
default:
