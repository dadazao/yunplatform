...
$(function () {
	if('${op}' != 'view'){
		editListTable("formColumnEditTable");
	}
	if('${op}'=='view'){
...
function editListTable(editDomId){
	...
	editcol: [{ colindex: 2, edittype: 2, ctrid: "columnId", css: ".input" },{colindex: 3}, { colindex: 4, edittype: 2, ctrid: "sel2", css: ".input" }, { colindex: 5 }, { colindex: 6 }, { colindex: 7, edittype: 2, ctrid: "sel4", css: ".input" } ,{ colindex: 8, edittype: 2, ctrid: "sel5", css: ".input" },{ colindex: 9, edittype: 2, ctrid: "sel6", css: ".input" },{ colindex: 10, edittype: 2, ctrid: "sel3", css: ".input" },{ colindex: 11, edittype: 2, ctrid: "sel1", css: ".input" }], //配置表格的编辑列 ctrid:为关联的dom元素id
		onok: function (obj) {
			...
			var isEdit = $(tds.get(7)).children().get(0).value;
			var isView = $(tds.get(8)).children().get(0).value;
			var required = $(tds.get(9)).children().get(0).value;
			var isDefaultQuery = $(tds.get(10)).children().get(0).value;
			var inputType = $(tds.get(11)).children().get(0).value;
			
			var params="formColumn.columnId="+columnId+"&formColumn.formOrder="+formOrder+"&formColumn.isShowInList="+isShowInList+"&formColumn.listOrder="+listOrder+"&formColumn.colWidth="+colWidth+"&formColumn.isEdit="+isEdit+"&formColumn.isView="+isView+"&formColumn.required="+required+"&formColumn.isDefaultQuery="+isDefaultQuery+"&formColumn.inputType="+inputType;
			var urlString = "<%=basePath %>/pages/resource/formsaveColumnAtList.action?formColumn.id=" + id+"&formColumn.formId=${formId}&formId=${formId}";
		...
		ondel: function (obj) {
			var tds=obj.children();
			var id = $(tds.get(0)).children().get(0).value;
			var urlString = "<%=basePath %>/pages/resource/formdeleteColumn.action?id="+id;
			$.ajax({
				type:'post',
				url: urlString,
				async: false,
...
function addRecord(){
	...
	$("#formColumnEditTable:first-child").prepend("<tr id='newtr'><td class='tdClass' width='3%'><input type='checkbox' style='margin-top:4px;*margin-top:0px;'  name='selectedSubIDs' value=''></td><td class='tdClass' width='3%'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'></td><td class='tdClass'><a href='#' style='text-decoration: none;'>详细设置</a></td></tr>");
	editListTable("newtr");
	...
	//控制查询条件和详细设置按钮
	enableLink($($(tds.get(10)).children().get(0)));
	//初始化
	enableTableSelect();
}
...
function autoAddRecord(){
	...
	var urlString="<%=basePath %>/pages/resource/formautoAddFormColumn.action?formId=${formId}&tabId="+tabid+"&partitionId="+partitionid+"&tableId="+tableid+"&relColumnId="+relColumnId;
	if(tabid=="-1"){
		alertMsg.info("请选择选项卡！");
		return false;
	}
...
function showDetailsPage(obj,inputType,id){
	...
	for(var m=0;m<trs.length;m++){
		...
		for(var n=0;n<tmptds.length;n++){
			$(tmptds[n]).removeClass("selectedTr");
		}
	}
	//设置tr背景颜色start
	for(var i=0;i<tds.length;i++){
		$(tds[i]).addClass("selectedTr");
	}
	//设置tr背景颜色end
	
	if($(tds.get(11)).find("select").html()==null){
		currentInputType=inputType;
	}else{
		currentInputType=$(tds.get(11)).children().get(0).value;
	}
	var kd = 600;
	var gd = 280;
	...
	$.pdialog.open("<%=basePath %>/pages/resource/formshowDetailPage.action?type="+currentInputType+"&id="+id+"&formId=${formId}&op=${op}","detailsDialog","详细设置",{width:kd,height:gd,mask:true,resizable:true});
}
...
function enableLink(obj,status){
	var inputType;
	if(status=="edit"){
		var obj = $(obj.children().get(11)).find("select").first();
		inputType = obj.val();
	}else{
		inputType = obj.value;
	}
	...
	if(inputType=='9'||inputType=='15'){
		$(tds.get(12)).html("<a style='color: gray;'>详细设置</a>");
	}else{
		if(tds.get(1).innerHTML==""){
			$(tds.get(12)).html("<a style='color: gray;'>详细设置</a>");
		}else{
			$(tds.get(12)).html("<a href=\"#\" style=\"color: blue;\" onclick=\"showDetailsPage(this,'"+inputType+"','"+id+"')\">详细设置</a>");	
		}
	}
	//不为以下类型时，不能作为查询条件
	if(inputType!='0' && inputType!='1' && inputType!='6' && inputType!='7' && inputType!='8'){
		$(tds.get(10)).html("<select id='sel3' name='formColumn.isDefaultQuery' style='width:75px;float:center;'><option value='0'>否</option></select><input type='hidden' value='否' />");
	}else{
		var dqvalue;
		if(id!=""){
			$.ajax({
				type:'post',
				url: "/pages/resource/formgetFormColumnObj.action?formColumnId="+id,
				async:false,
				dataType: 'json',
				success: function(data){
					var formColumn = data;
		...
		if(dqvalue==0){
			$(tds.get(10)).html("<select id='sel3' name='formColumn.isDefaultQuery' style='width:75px;float:center;'><option value='1'>是</option><option value='0' selected='selected'>否</option></select><input type='hidden' value='否' />");
		}else if(dqvalue==1){
			$(tds.get(10)).html("<select id='sel3' name='formColumn.isDefaultQuery' style='width:75px;float:center;'><option value='1' selected='selected'>是</option><option value='0'>否</option></select><input type='hidden' value='是' />");
		}
	}
}
...
<form id="pagerForm" method="post" action="<%=basePath %>/pages/resource/formlistColumn.action?formId=${formId}&tabId=${tabId}&partitionId=${partitionId}">
...
<div id="formcolumn_list_divid">
    ...
    <table id="formColumnEditTable" width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" align="center" class="tableClass" style=" border-left:1px #ededed solid;">
		<tbody>
		<thead>
			<tr>
				<th class="thClass" width="3%"><input name="all" type="checkbox"  style="margin-top:4px;*margin-top:0px;" onclick="dialogSelectAll(this,'selectedSubIDs')"/></th>
	        ...
	        <c:if test="${status.count%2==0}">
               ...
               <tr target="id_column" rel="1">
	             ...
	             <td class="tdClass" style="text-align: center;" onmousedown="editTable(this)">${column.columnZhName}</td>
             	 <td class="tdClass" onmousedown="editTable(this)">${column.formOrder}</td>
	             ...
	             <td class="tdClass" onmousedown="editTable(this)">
	             	<c:if test="${column.isEdit==1 }">是</c:if>
	             	<c:if test="${column.isEdit==0 }">否</c:if>
	             </td>
	             <td class="tdClass" onmousedown="editTable(this)">
	             	<c:if test="${column.isView==1 }">是</c:if>
	             	<c:if test="${column.isView==0 }">否</c:if>
	             </td>
	             <td class="tdClass" onmousedown="editTable(this)">
	             	<c:if test="${column.required==1 }">是</c:if>
	             	<c:if test="${column.required==0 }">否</c:if>
	             </td>
	             <td class="tdClass" onmousedown="editTable(this)">
