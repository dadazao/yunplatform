	...
	<tr>
		...
		<td align="left" colspan="3">
			<select name="formColumn.compexId" style="width:175px">
				<c:forEach items="${components }" var="searchCombox">
					<option value="${searchCombox.id}" <c:if test="${formColumn.compexId == searchCombox.id}">selected="selected"</c:if>>${searchCombox.searchComboxName}</option>				
				</c:forEach>
			</select>
		</td>
	</tr>
	<tr id="selectDataTypeId">
			 <td height="30" class="Input_Table_Label" width="18%" align="left"><label id="selectDataTypeLable" >数据来源</label></td>
