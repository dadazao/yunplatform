/**
 * @author ZhangHuihua@msn.com
 */
(function($){
	// jQuery validate
	$.extend($.validator.messages, {
		required: "必填字段",
		remote: "请修正该字段",
		email: "请输入正确格式的电子邮件",
		url: "请输入合法的网址",
		date: "请输入合法的日期",
		dateISO: "请输入合法的日期 (ISO).",
		number: "请输入合法的数字",
		digits: "只能输入整数",
		creditcard: "请输入合法的信用卡号",
		equalTo: "请再次输入相同的值",
		accept: "请输入拥有合法后缀名的字符串",
		maxlength: $.validator.format("长度最多是 {0} 的字符串"),
		minlength: $.validator.format("长度最少是 {0} 的字符串"),
		rangelength: $.validator.format("长度介于 {0} 和 {1} 之间的字符串"),
		range: $.validator.format("请输入一个介于 {0} 和 {1} 之间的值"),
		max: $.validator.format("请输入小于等于{0}的值"),
		min: $.validator.format("请输入大于等于{0}的值"),
		
		alphanumeric: "字母、数字、下划线",
		lettersonly: "必须是字母",
		phone: "数字、空格、括号",
		
		//固定电话、传真、直拨电话号码【格式（X为0、3、4位，Y为3～8位，Z为0～4位）：XXXX-YYYYYYYY-ZZZZ、XXXX YYYYYYYY ZZZZ、XXXXYYYYYYYYZZZZ】
		telephone: "请输入正确的电话号码",
		//移动电话号码【格式（X为数字）：13XXXXXXXXX、15XXXXXXXXX、18XXXXXXXXX】
		mobile: "请输入正确的手机号码",
		//六位邮政编码【格式（X位数字）：XXXXXX】
		postcode: "请输入正确的邮政编码",
		//一代、二代身份证号码【格式（A为户口所在地省市区/县代码编号，B为2、4位出生年份，C为出生月份，D为出生日期，E为顺序编号，F为二代身份证的校验码）：AAAAAABBBBCCDDEEEF】
		identitycard: "请输入正确的身份证号码",
		//银行卡（包括储蓄卡、信用卡等银行卡），至少包含16位数字，支持空格输入
		bankcard: "请输入正确的银行卡号码",
		//只能是   数字，字母，下划线，横杠
		pinyin: "请输入正确的拼音名称",
		multiEmail:"请确保输入的邮箱地址全都合法"
	});
	
	// DWZ regional
	$.setRegional("datepicker", {
		dayNames: ['日', '一', '二', '三', '四', '五', '六'],
		monthNames: ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']
	});
	$.setRegional("alertMsg", {
		title:{error:"错误", info:"提示", warn:"警告", correct:"成功", confirm:"确认提示"},
		butMsg:{ok:"确定", yes:"是", no:"否", cancel:"取消"}
	});
})(jQuery);