	...
	$.pdialog = {
		...
		open:function(url, dlgid, title, options) {
			...
			if (op.mask) {
				...
				$(dialog).css("zIndex", 1000+100*currentLevel);
				$("a.minimize",dialog).hide();
				$(dialog).data("mask", true);
				//$("#dialogBackground").css("z-index", 1000+100*(currentLevel-1)+50);
				$("#dialogBackground").attr("style","width:100%;height:100%;z-index:" + (1000+100*(currentLevel-1)+50) + ";");
				//alert("width:100%;height:100%;z-index:" + (1000+100*(currentLevel-1)+50) + ";");
		...
		close:function(dialog) {
			...
			if(currentLevel<=0) {
				...
				$("#dialogBackground").css("zIndex", 1000+100*(currentLevel-1)+50);	
			}
			//关闭搜索下拉框
			$('ul[role="listbox"]').hide();
			$('#menu_tblContent_tblContent_styleselect_menu').hide();
			$('#menu_tblContent_tblContent_formatselect_menu').hide();
			$('#menu_tblContent_tblContent_fontselect_menu').hide();
			$('#menu_tblContent_tblContent_fontsizeselect_menu').hide();
			$('#tblContent_backcolor_menu').hide();
			$('#tblContent_forecolor_menu').hide();
		},
