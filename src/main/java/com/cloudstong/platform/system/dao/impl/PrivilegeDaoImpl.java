...
public class PrivilegeDaoImpl implements IPrivilegeDao {
	...
	public int queryPrivilegeNum(String catalogId){
		return jdbcTemplate.queryForObject("select count(*) from sys_privilege where tbl_catalog=?",new Object[] {catalogId}, Integer.class);
	}
	
	public boolean hasDelPrivilege(String bianma){
		return (jdbcTemplate.queryForObject("select count(*) from sys_privilege where tbl_seq in (select id from sys_bianma where tbl_value = ?)",new Object[] {bianma.split(",")[0]}, Integer.class))>0 ? true :false;
	}
}
