...
<!--
	ns.org.loadAllUser = function(){
		$("#add_all_user").loadUrl("<%=basePath %>/pages/system/user/userpUserList.action?relationId=1");
	}
	
	ns.org.closePAddUser = function(){
		$.pdialog.close("addUserDialog");
	}
	
	ns.org.savePAddUser = function(){
		var ids = '';
		$.each($("div[belong='usersId']"),function (entryIndex,entry){
			ids += $(entry).html()+";";
		});
		$.ajax({
			url : "<%=basePath %>/pages/system/org/orgaddUser.action",
			type: 'post',
			data: 'userIds='+ids+'&orgId=${param.orgId}',
			dataType : "json",
			success : function(data) {
				alertMsg.info("操作成功!");
				if(${param.forward=='setUserList'}){
					ns.org.loadSetUserTabList('${param.orgId}');
				}else{
...
<div class="buttonPanel" align="center">
	<button type="button" class="listbutton" onclick="ns.org.savePAddUser()">
		确定
	</button>
	<button type="button" class="listbutton" onclick="ns.org.closePAddUser()">
		取消
	</button>
</div>