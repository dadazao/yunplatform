...
$.fn.editable = function (options) { 
	...
	var button = "<td class='tdClass' width='7%'><a href='#' class='" + opt.editcss + "'>编辑</a> <a href='#' class='" + opt.delcss + "'>删除</a><a href='#' class='" + opt.onokcss + "'>保存</a> <a href='#' class='" + opt.canclcss + "'>取消</a></td>"; 
	...
	if(opt.editcolumn==false){
		$trs.find(".onok, .cancl, .edit, .del").hide();
	}
$trs.find(".edit").click(
	function () { 
		$tr = $(this).closest("tr"); 
					...
					if (obj.colindex == i) { 
						css = obj.css ? "class='" + obj.css + "'" : ""; 
							if (obj.edittype == undefined || obj.edittype == 0) { 
								...
								//$(td).find("option").filter(":contains('" + t + "')").first().attr("selected", true);
