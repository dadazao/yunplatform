...
var navTab = {
	...
	_switchTab: function(iTabIndex){
		...
		var preId = "20121126"+new Date().getTime()+Math.floor(Math.random()*10000);
		var $oPanel = this._getPanels().eq(this._lastIndex);
		$("*[id]",$oPanel).each(function(){
			var $e = $(this);
			var id = $e.attr("id");
			var idbak = $e.attr("idbak");
			if(id && id.indexOf("combox_")==-1) {
				$e.attr("idbak",preId+"______"+id);
				$e.attr("id","");
			}
		});	 
		var $nPanel = this._getPanels().eq(iTabIndex);
		$("*[id]",$nPanel).each(function(){
			var $e = $(this);
			var id = $e.attr("id");
			var idbak = $e.attr("idbak");
			if(idbak) {
				var num = idbak.indexOf("______");
				if(num != -1) {
					idbak = idbak.substr(num+6);
				}
				$e.attr("id",idbak);
			}
		});	
