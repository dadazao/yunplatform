...
function validateCallback(form, callback) {
	...
	if(enctype=="multipart/form-data"){
		return iframeCallback(form,selfIframeDialogAjaxDone);
	}else{
		if (!$form.valid()) {
			return false;
		}
...
function _iframeResponse(iframe, callback){
	...
	$iframe.bind("load", function(event){
		...
		if (doc.XMLDocument) {
			...
			try{
				response = $iframe.contents().find("body").text(); //.html()当后台返回JSON字符串时会有错误
...
function refreshList(json) {
	modifyCurrentPage();
	DWZ.ajaxDone(json);
	$(".goto",navTab.getCurrentPanel()).each(function(){
		var $this = $(this);
		$this.click();
	});
}
...
function modifyCurrentPage(){
	$(".pagination").each(function(i){
		...
		var pageSize = parseInt($this.attr("numPerPage"));
		var currentPage = parseInt($this.attr("currentPage"));
		if(currentPage == Math.floor((totalCount+pageSize-1)/pageSize))  {
			var lastNum = totalCount%pageSize;
			if($this.attr("targetType")=='dialog'){
				if($("input[name='selectedSubVOs']",$this.parent().next()).size()==lastNum){
					$(".jumpto",$this).children().first().val((currentPage-1)==0?1:(currentPage-1));
				}
				if($("input[name='selectedSubIDs']",$this.parent().next()).size()==lastNum){
					$(".jumpto",$this).children().first().val((currentPage-1)==0?1:(currentPage-1));
				}
			}else{
				if($("input[name='selectedVOs']").size()==lastNum){
					$(".jumpto",$this).val((currentPage-1)==0?1:(currentPage-1));
				}
			}
		}
	});
...
function dialogRefresh() {
	$(".goto",$.pdialog.getCurrent()).each(function(){
		var $this = $(this);
		$this.click();
	});
}
