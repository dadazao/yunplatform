...
function initLayout(){
	...
	var iContentH = $(window).height() - $("#header").height() - 30;

	$("#container").width(iContentW);
	$("#desktop").width(iContentW);
	$("#desktop").height(iContentH-28);
	...
	$("#nav_word").width(wWidth - 7 - 7);
}

function getContentWidth() {
	return $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() + 10 : 34) - 5;
}

function getContentHeight() {
	return $(window).height() - $("#header").height() - 30;
}
...
function initAccordion(){
	$('div.accordion', $(document)).each(function(){
		var $this = $(this);
		$this.accordion({fillSpace:$this.attr("fillSpace"),alwaysOpen:true,active:0});
	});
}
...
function initPagination() {
	$("div.pagination", $(document)).each(function(){
		var $this = $(this);
		$this.pagination({
			targetType:$this.attr("targetType"),
			rel:$this.attr("rel"),
...
function initDate() {
	$('input.date', $(document)).each(function(){
		var $this = $(this);
		var opts = {};
		if ($this.attr("format")) opts.pattern = $this.attr("format");
		if ($this.attr("yearstart")) opts.yearstart = $this.attr("yearstart");
