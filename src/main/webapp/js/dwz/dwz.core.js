...
var DWZ = {
	...
	ajaxError:function(xhr, ajaxOptions, thrownError){
		if (alertMsg) {
			...
			html = html.replace(/margin-top:100px;/, "margin-top:10px;");
			alertMsg.open(html);
		} else {
			alert("系统错误！");
		}
	...
	ajaxDone:function(json){
		...
		if(json.statusCode == DWZ.statusCode.error) {
			if(json.message && alertMsg) alertMsg.error(json.message, json.options, json.title);
		} else if (json.statusCode == DWZ.statusCode.timeout) {
			if(alertMsg) alertMsg.error(json.message || DWZ.msg("sessionTimout"), {okCall:DWZ.loadLogin}, json.title);
			else DWZ.loadLogin();
		} else {
			if(json.message && alertMsg) alertMsg.correct(json.message, json.options, json.title);
		};
	},
