...
<!--
	ns.role.loadAllUser = function(){
		$("#add_all_user").loadUrl("<%=basePath %>/pages/system/user/userpUserList.action?relationId=1");
	}
	
	ns.role.closePAddUser = function(){
		$.pdialog.close("addUserDialog");
	}
	
	ns.role.savePAddUser = function(){
		var ids = '';
		$.each($("div[belong='usersId']"),function (entryIndex,entry){
			ids += $(entry).html()+";";
		});
		$.ajax({
			url : "<%=basePath %>/pages/system/role/roleaddUser.action",
			type: 'post',
			data: 'userIds='+ids+'&roleId=${param.roleId}',
			dataType : "json",
...
<div class="buttonPanel" align="center">
	<button type="button" class="listbutton" onclick="ns.role.savePAddUser()">
		确定
	</button>
	<button type="button" class="listbutton" onclick="ns.role.closePAddUser()">
		取消
	</button>
</div>