...
<!--
ns.position.addInUserList = function(obj,id,name){
	if(obj.checked){
		var flag = true;
		...
		if(flag){
			$("#selectUserList").append("<tr id='"+id+"'><td height='14px' align='center'>"+name+"<div style='display:none' belong='usersId'>"+id+"</div></td><td height='14px' align='center'><div onclick='ns.position.deleteTr(this)' style='cursor: pointer;width:12px;height:12px; background-image: url(\"<%=basePath %>/themes/css/images/ui-icons_cd0a0a_256x240.png\");background-repeat: no-repeat;background-position: -99px -131px;'></div></td></tr>");
		}
	}else{
		$("#"+id).remove();
	}
...
ns.position.deleteTr = function(entry){
...
<tbody>
	...
	<c:forEach items="${pageResult.content}" var="puser" varStatus="status">
	     <c:if test="${status.count%2==0}">
	          ...
	          <td class="tdClass" width="5%"><input onclick="ns.position.addInUserList(this,'${puser.id}','${puser.fullname}')" type="checkbox" class="checkbox" value="${puser.id}"></td>
