...
<script type="text/javascript">
	ns.bpm.delProcessRunHistory = function(id) {
		var url;
		if(id != undefined) {
			...
			url = "<%=basePath %>/pages/third/bpm/processRun/del.action?" + param;
		}
		alertMsg.confirm("确定要删除吗?", {okCall:function(){ajaxTodo(url,refreshList);}});
	}
	
	ns.bpm.viewProcessRunHistory = function(id) {
		var url = "<%=basePath %>/pages/third/bpm/processRun/view.action?runId=" + id;
	...
	ns.bpm.logProcessRunHistory = function(id) {
...
<body>	
	<form id="pagerForm" method="post" action="<%=basePath %>/pages/third/bpm/processRun/history.action">
	...
	<div class="pageContent">
		<div class="panelBar">
			<table>
				<tr><td>
					<button type="button" id="exportXml" name="exportXml" class="listbutton" onclick="ns.bpm.delProcessRunHistory();" >删除</button>
				</td></tr>
			</table>
		</div>
	</div>
	<div id="defaultQuery" class="pageHeader">
		<div class="searchBar">
			<form id="defaultQueryForm" onsubmit="return navTabSearch(this);" action="<%=basePath %>/pages/third/bpm/processRun/history.action" method="post">
		            ...
		            <c:forEach items="${pageResult.content}" var="processRunItem" varStatus="status">
		                ...
		                <c:if test="${status.count%2!=0}">
		                	<tr target="id_column" rel="1">
		                    	...
		                    	<td align="center">
		                    		<a style="cursor: pointer;color:blue;" onclick="ns.bpm.viewProcessRunHistory(${processRunItem.runId})" >明细</a>&nbsp;
		                    		<a style="cursor: pointer;color:blue;" onclick="ns.bpm.delProcessRunHistory(${processRunItem.id})"  >删除</a>&nbsp;
									<a style="cursor: pointer;color:blue;" onclick="ns.bpm.logProcessRunHistory(${processRunItem.id})" >操作日志</a>
		                    	</td>
		                    </tr>
		            </c:forEach>
				</tbody>	
