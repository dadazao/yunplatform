...
<script type="text/javascript">	
	function loadButton(str) {
		var urlString = "<%=basePath %>/pages/resource/fbutton/addButton.action?formId="+$("#formId").val()+"&op="+str;
		$('#buttonId').load(urlString);
	}
	function loadButtonList(str) {
		var urlString = "<%=basePath %>/pages/resource/fbutton/listButton.action?formId=" + $("#formId").val()+"&op="+str;
	...
	function loadEditFormButton(id,str) {
		var urlString = "<%=basePath %>/pages/resource/fbutton/edit.action?formId="+$("#formId").val()+"&buttonId=" + id+"&op="+str+"&marking=list";
		$('#buttonId').load(urlString);	
	}
	function addFormButton() {
		...
		$("#btnId").val("");
		var urlString = "<%=basePath %>/pages/resource/fbutton/save.action?formId=" + $("#formId").val();
		$.ajax({
			type:'post',
			url: urlString,
	...
	function deleteFormButton() {
		...
		alertMsg.confirm("确定要删除吗?", {okCall:function(){
				var urlString = "<%=basePath %>/pages/resource/fbutton/del.action?formId=" + $("#formId").val();
				$.ajax({
					type:'post',
					url: urlString,
	...
	function showButton(v){
		var urlString = "<%=basePath %>/pages/resource/fbutton/showButton.action?buttonType=" + v;
		$.ajax({
			type:'post',
			url: urlString,
			success: function(data){
	...
	function loadTab(str) {
		var urlString = "<%=basePath %>/pages/resource/tab/add.action?op="+str;
		$('#tabId').loadUrl(urlString);
	}
	function loadTabList(str) {
		var urlString = "<%=basePath %>/pages/resource/tab/list.action?formId=" + $("#formId").val()+"&op="+str;
		$.ajaxSetup({async: false});
	...
	function loadEditTab(id,str) {
		var urlString = "<%=basePath %>/pages/resource/tab/edit.action?id=" + id+"&op="+str;
		$('#tabId').loadUrl(urlString);	
	}
	function addTab() {
		$("#tId").val("");
		...
		var urlString = "<%=basePath %>/pages/resource/tab/save.action?formId=" + $("#formId").val();
		$.ajax({
			type:'post',
			url: urlString,
	...
	function showTab() {
		var urlString = "<%=basePath %>/pages/resource/tab/show.action?formId=" + $("#formId").val();
		$.ajax({
			type:'post',
			url: urlString,
			success: function(data){
	...
	function deleteTab() {
		...
		alertMsg.confirm("确定要删除吗?", {okCall:function(){
				var urlString = "<%=basePath %>/pages/resource/tab/del.action?formId=" + $("#formId").val();
				$.ajax({
					type:'post',
					url: urlString,
	...
	function loadFormColumn(str) {
		var urlString = "<%=basePath %>/pages/resource/form/addColumn.action?formId=" + $("#formId").val()+"&op="+str;
	...
	function loadFormColumnList(tabid,partitionid,tableid,str) {
		...
		var urlString = "<%=basePath %>/pages/resource/form/listColumn.action?formId=" + $("#formId").val()+"&tabId="+tabid+"&partitionId="+partitionid+"&tableId="+tableid+"&op="+str;
		$.ajaxSetup({async: false});
	...
	function loadEditFormColumn(id) {
		var urlString = "<%=basePath %>/pages/resource/form/editColumn.action?id=" + id + "&formId=" + $("#formId").val();
		$('#formColumnId').loadUrl(urlString);	
	}
	function addFormColumn() {
		...
		$("#fcId").val("");
		var urlString = "<%=basePath %>/pages/resource/form/saveColumn.action?formId=" + $("#formId").val();
...
<%--	  		type:'POST',--%>
<%--	  		async: false,--%>
<%--	  		url:'<%=basePath %>/pages/resource/form/isCanDelete.action',--%>
...
<%--		}--%>
		alertMsg.confirm("确定要删除吗?", {okCall:function(){
				var urlString = "<%=basePath %>/pages/resource/form/deleteColumn.action?formId=" + $("#formId").val();
				$.ajax({
					type:'post',
					url: urlString,
					data:$("#formColumnListForm").serialize(),
	...
	function showColumn(){
		var urlString = "<%=basePath %>/pages/resource/form/showColumn.action?id=" + $("#tableId").val();
		$.ajax({
			type:'post',
			url: urlString,
			success: function(data){
	...
	function setDefaultQuery(columnId){
		var urlString = "<%=basePath %>/pages/resource/form/setDefaultQuery.action?id=" + columnId+"&formId=" + $("#formId").val();
		$.ajax({
			type:'post',
			url: urlString,
			success: function(data){
	...
	$(function(){
		...
		xjUrl="<%=basePath %>/pages/resource/form/add.action?op=new&formManageId=${formManageId}";
		bcUrl="<%=basePath %>/pages/resource/form/save.action";
		plscUrl="<%=basePath %>/pages/resource/form/del.action?model=biaodan";
		ljscUrl="<%=basePath %>/pages/resource/form/logicDelete.action?model=biaodan";
...
<body>
	<form id="pagerForm" method="post" action="<%=basePath %>/pages/resource/form/list.action">
	...
	<div id="defaultQuery" class="pageHeader" style="height: ${tabulation.queryControl.queryControlHeight==0 ? '' : tabulation.queryControl.queryControlHeight}px;width:${tabulation.queryControl.queryControlWidth}%">
		<div class="searchBar">
			<form onsubmit="return navTabSearch(this);" action="<%=basePath %>/pages/resource/form/list.action?queryTpe=0" method="post">
							...
							<td align="center">
								<c:forEach items="${tabulation.tabulationOpts}" var="tabulationOpt" varStatus="ops">
									<c:if test="${tabulationOpt.hasAuth=='0' && ops.count<=tabulation.listControl.operationManage.optManagerCount}">
										&nbsp;
										<a style="cursor: pointer;" onclick="eventCompex${tabulationOpt.button.buttonBM}('<%=basePath %>/pages/resource/form/view.action','id=<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == \"id\"}">${ce.value}</c:if></c:forEach>&formManageId=${formManageId}')"><span><font style="font-family:${tabulationOpt.button.buttonNameFontStyle};font-size: ${tabulationOpt.button.buttonNameFontSize}px;color:${tabulationOpt.button.buttonNameFontColor}">${tabulationOpt.showName}</font></span></a>
										&nbsp;
									</c:if>
								</c:forEach>
							</td>
