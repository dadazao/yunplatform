...
%>
<div id="compId">
<form id="tempForm" method="post" action="pages/resource/formsaveColumnDetails.action" class="pageForm required-validate" onsubmit="return validateCallback(this, detailDialogAjaxDone);">
	...
	<tr id="textBoxId">
		...
		<td align="left" colspan="3">
			<select name="formColumn.compexId" style="width: 175px">
				<c:forEach items="${components}" var="uploadbox">
					<option value="${uploadbox.id}"
						<c:if test="${formColumn.compexId==uploadbox.id }">selected="selected"</c:if>>
						${uploadbox.uploadBoxName}
					</option>
				</c:forEach>
			</select>
		</td>
