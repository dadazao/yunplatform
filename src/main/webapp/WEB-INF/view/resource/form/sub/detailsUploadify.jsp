...
<table width="100%" cellspacing="0" cellpadding="2" border="0" class="Input_Table">
	...
	<tr id="textBoxId">
		<td class="Input_Table_Label" align="left" width="18%">
			<label>
				 多文件上传组件
			</label>
		</td>
		<td align="left" width="32%">
			<select name="formColumn.compexId" style="width: 175px">
				<c:forEach items="${uploadifys}" var="uploadify">
					<option value="${uploadify.id}"
						<c:if test="${formColumn.compexId==uploadify.id }">selected="selected"</c:if>>
						${uploadify.uploadifyName}
					</option>
				</c:forEach>
			</select>
		</td>
		...
		<td align="left" width="35%">
			<select name="formColumn.recordColumn" style="width: 175px">
				<option value="">无</option>
				<c:forEach items="${recordColumns}" var="column">
					<option value="${column.id}"
						<c:if test="${formColumn.recordColumn==column.id }">selected="selected"</c:if>>
						${column.columnZhName}
					</option>
				</c:forEach>
			</select>
