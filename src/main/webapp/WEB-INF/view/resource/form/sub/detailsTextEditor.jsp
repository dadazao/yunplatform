	...
	<tr id="textBoxId">
		...
		<td align="left" colspan="3">
			<select name="formColumn.compexId" style="width: 175px">
				<c:forEach items="${textEditors}" var="textEditor">
					<option value="${textEditor.id}"
						<c:if test="${formColumn.compexId==textEditor.id }">selected="selected"</c:if>>
						${textEditor.textEditorName}
					</option>
				</c:forEach>
			</select>
		</td>
