		                	...
		                	<c:if test="${tabulation.listControl.selectId!='-1'&&tabulation.listControl.selectId!=null}">
		                    	<td align="${tabulation.listControl.selectManage.selectManagePosition}">
		                    		<input 
		                    			...
		                    			<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == 'comm_status' && ce.value==1}">disabled</c:if></c:forEach>
		                    		/>
		                    	</td>
		                    </c:if>
		                    <c:if test="${tabulation.listControl.orderId!='-1'&&tabulation.listControl.orderId!=null}">
		                    ...
		                    <c:if test="${tabulation.listControl.operationId!='-1'&&tabulation.listControl.operationId!=null}">
								<td align="center">
											...
											<c:forEach items="${tabulation.tabulationOpts}" var="tabulationOpt" varStatus="ops">
												...
												<c:choose>
													...
													<c:when test="${tabulationOpt.button.buttonName=='查看文件'}">
														&nbsp;
														<a style="cursor: pointer;" onclick="eventCompex${tabulationOpt.button.buttonBM}('<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == \"id\"}">${ce.value};</c:if></c:forEach>')"><span><font style="font-family:${tabulationOpt.button.buttonNameFontStyle};font-size: ${tabulationOpt.button.buttonNameFontSize}px;color:${tabulationOpt.button.buttonNameFontColor}">${tabulationOpt.showName}</font></span></a>
														&nbsp;
													</c:when>
													<c:otherwise>
														&nbsp;
									...
									<c:if test="${commStatus==0}">
										<a style="cursor: pointer;" onclick="eventCompexLock('<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == \"id\"}">${ce.value}</c:if></c:forEach>')"><span><font color="blue">加锁</font></span></a>
									</c:if>
									<c:if test="${commStatus==1}">
										<a style="cursor: pointer;" onclick="eventCompexUnLock('<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == \"id\"}">${ce.value}</c:if></c:forEach>')"><span><font color="blue">解锁</font></span></a>
									</c:if>
									&nbsp;
									<!-- 
									<a style="cursor: pointer;" onclick="eventCompexLSBB('<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == \"id\"}">${ce.value}</c:if></c:forEach>')"><span><font color="blue">历史版本</font></span></a>
									&nbsp;
									-->
									<a style="cursor: pointer;" onclick="eventCompexConfigColumns('<c:forEach items="${dom.tabulationColumnExtends}" var="ce" ><c:if test="${ce.formColumn.columnName == \"id\"}">${ce.value}</c:if></c:forEach>')"><span><font color="blue">设置</font></span></a>
								</td>
							</c:if>
		                </tr>
		            </c:forEach>
