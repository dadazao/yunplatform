...
<script type="text/javascript">
	$(function(){
		partitionListDiv='${partitionListDiv}';
		tjTabUrl="<%=basePath %>/pages/resource/${simpleModel}compexsavesub.action";
		bcTabUrl="<%=basePath %>/pages/resource/${simpleModel}compexsavesub.action";
		scTabUrl="<%=basePath %>/pages/resource/${simpleModel}compexdeletesub.action?formId=${formId}";
		whTabUrl="<%=basePath %>/pages/resource/${simpleModel}compexeditsub.action?";
...
<body>
	<form id="pagerForm" method="post" action="<%=basePath %>/pages/resource/${simpleModel}compexlistsub.action">
		...
		<input type="hidden" name="partitionListDiv" value="${partitionListDiv}"/>
	</form>
	...
	<form id="${partitionListDiv}Form" name="${partitionListDiv}Form" method="post">
				...
				<thead>
					<tr>
						<th class="thClass" style=" border-bottom:1px #d0d0d0 solid; border-right:1px #d0d0d0 solid;">
							<input name="all" type="checkbox"  class="checkbox" onclick="dialogSelectAll(this,'selectedSubVOs')"/>
						</th>
	            ...
	            <c:forEach items="${pageResult.content}" var="dom" varStatus="status">
	                ...
	                <c:if test="${status.count%2!=0}">
                    	...
                    	<td class="tdClass" style="width: 5%; border-bottom:1px #ededed solid; border-right:1px #ededed solid;">
                    		<input type="checkbox" class="checkbox"  name="selectedSubVOs" value="${dom.table.tableName }-id:${dom.id}">
                    	</td>
						...
						<td class="tdClass" style="width: 10%; border-bottom:1px #ededed solid; border-right:1px #ededed solid;">
							 &nbsp;
							 <a onclick="loadCompexEditSubList('${tabId}','${dom.id}','${partitionListDiv}','${op}');" style="cursor: hand"><span>维护</span></a>
							 &nbsp;
						</td>
	                </tr>
	            </c:forEach>
