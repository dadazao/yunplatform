...
<!--
	ns.email.viewMailMessage= function(id){
		navTab.openTab('viewMessageTab'+id,'<%=basePath%>/pages/platform/email/mailMessage/view.action?mailMessage.id='+id,{title:'已发邮件'});
	}
	ns.email.deleteMailMessage = function(id){
		if(id){
...
<body>	
	<form id="pagerForm" method="post" action="<%=basePath%>/pages/platform/email/mailMessage/outbox.action?queryType=${queryType}&email=${email}">
		...
		<div class="panelBar">
			<table>
				<tr><td>
					...
					<button type="button" class="listbutton" onclick="ns.email.deleteMailMessage();" >删除</button>				
				</td></tr>
			</table>
		</div>
	</div>
	<div id="defaultQuery" class="pageHeader">
		<div class="searchBar">
			<form id="defaultQueryForm" onsubmit="return divSearch(this,'mailBoxListArea');" action="<%=basePath%>/pages/platform/email/mailMessage/outbox.action?queryType=0&email=${email}" method="post">
		            ...
		            <c:forEach items="${pageResult.content}" var="mailMessage" varStatus="status">
		                ...
		                <c:if test="${status.count%2!=0}">
		                	<tr target="id_column" rel="1">
		                    	...
		                    	<td align="center" title="${mailMessage.to}"><div style="text-align:left;">${mailMessage.to}</div></td>
		                    	...
		                    	<td>
		                    		...
		                    		<a style="cursor: pointer;color:blue;" onclick="ns.email.deleteMailMessage('${mailMessage.id}');">删除</a>
		                    	</td>
		                    </tr>
		            </c:forEach>
				</tbody>	
