...
public class CatalogServiceTestCase{
	...
	@Test
	public void testUpdateCatalogPath() {
		...
		for(Map<String,Object> o : os) {
			...
			if(path.contains("compexlist.action")) {
				path = path.replace(listId.toString()+"compexlist.action", EncryptUtil.Md5(zhubiao)+"compexlist.action");
	...
	@Test
	public void testUpdateResourcePath() {
